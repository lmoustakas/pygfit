...
for ( filt, filt_data ) in sims.iteritems():
	...
	cat = cat[ cat['model'] == 'sersic' ]
	
	mag_diff = cat['mag_input']-cat['mag']
	
	xs = [ cat['mag_input'], cat['n'], cat['re_arcsecs'], cat['re_arcsecs']/cat['nearest'] ]
	xmins = [12, 0, 0, 0]
	xmaxs = [19, 8, 2.0, 1.0]
	xlabels = ['Mag In',r'$n$',r'$r_e$ (")',r'$r_e$/Separation']
	xticklabelformats = ['%.0f', '%.0f', '%.1f', '%.1f']
	nxs = len( xs )
	...
	pyplot.savefig( 'ps/%s_error_correlations_sersic.eps' % filt )
	pyplot.clf()