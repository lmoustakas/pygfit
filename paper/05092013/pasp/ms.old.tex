...
\slugcomment{Submitted to PASP, August 16, 2013}
...
We apply \pygfit{} to real images and also perform simulations to measure \pygfit{}'s fidelity.  With the exception of some small residuals in the two bluest IRAC filters where the PSF is undersampled, \pygfit{} is able to cleanly subtract galaxies from the LRI.  Especially in the ground-based images, where the PSF is well resolved, there appears to be nothing left in the residual images but sky noise.  Simulations show that the uncertainty in \pygfit{}'s magnitudes are consistent with being limited by sky noise.
