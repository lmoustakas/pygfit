...
class point(object):
	...
	def generate_model( self, pad, img_shape, psf, shift=True ):
		...
		# to be entirely correct the image should be shifted since the object probably doesn't fall in the center of the pixel,
		# but since the fitting process will move the image anyway, we're going to skip an extra interpolation step.
	...
	def get_catalog( self, params, return_parameters=False ):
		...
		cat = {'hres_id': [self.id], 'model': [self.model_type], 'img_x': [new_x], 'img_y': [new_y], 'flux': [params[2]], 'mag': [self.flux_to_mag( params[2] )], 'mag_image': [mag_image], 'mag_initial': [self.mag_initial], 'mag_hres': [self.mag], 're_lres': [self.re], 'n': [self.n], 'pa': [self.pa], 'ba': [self.ba], 'ra': [self.ra], 'dec': [self.dec]}
